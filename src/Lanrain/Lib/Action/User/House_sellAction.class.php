<?php
class House_sellAction extends UserAction{
	...
	public function edit() {
		...
		if (IS_POST) {
			...
			if ($_POST['id'] != "") {
					...
					if ($result !== false) {
						//相册更新
						M('house_img')->where(array('item_id'=>$data['id']))->delete();
						$_img['item_id'] = $data['id'];
						$_img['add_time'] = time();
						foreach ($imgs as $oneimg){
							$_img['url'] = $oneimg;
							M('house_img')->add($_img);
						}
						$this->success('成功！', U('House_sell/index'));
					} else {
						$this->error('失败！');
					}
			} else {
		...
		//非提交，有id为编辑展示，无id为新增展示
			...
			if ($id) {
				...
				$imgDetail = M("house_img")->where(array("item_id"=>$id))->order("id ASC")->select();
				$index = 1;
