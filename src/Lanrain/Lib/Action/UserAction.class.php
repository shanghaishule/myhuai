...
class UserAction extends BaseAction{
	protected function _initialize(){
		...
		$wherearr = array();
		$wherearr['belonguser'] = session('belonguser');
		$myfun = M('myfun')->where(array('users'=>session('uid')))->select();
		$myfunstr = '';
		foreach($myfun as $onefun){$myfunstr .= $onefun['funtype'].',';}
		//dump($myfunstr);exit;
		$wherearr['funtype'] = array('in',$myfunstr);
		$allfunction = M('Function')->where($wherearr)->order('id')->select();
		$allfunctiontype = M('Function')->Distinct(true)->field('funtype')->where($wherearr)->order('usenum')->select();
